...
inline int HowManyThreads(int max_num_threads, int rows, int cols, int depth) {
  ...
  std::cout << "HowManyThreads 1 \n";
  std::cout << "max_count " << max_count << "\n";
  std::cout << "kernel rows " << KernelRows << "\n";
  ...
  std::cout << "rows " << std::uint64_t(rows) << "cols " << std::uint64_t(cols) \
         << " depth " << std::uint64_t(depth) << "\n";
  std::cout << "HowManyThreads 5 \n";
    //minsung_handle
    thread_count =
        std::min(thread_count, int(cubic_size / min_cubic_size_per_thread));
  std::cout << "HowManyThreads 6 \n";
  std::cout << "thread_count " << thread_count << "\n";
    if (thread_count < 1) {
      thread_count = 1;
    }
...
void MultiThreadGemm(GemmContextType* context, const KernelBase& kernel,
  ...
  std::cout << "MultiTHreadGemm \n";
  std::cout << "lhs.cols : " << lhs.cols() << "\n";
  assert(lhs.cols() == rhs.rows());
  std::cout << "lhs.cols after assert : " << lhs.cols() << "\n";
  ...
  std::cout << "MultiTHreadGemm 2.5\n";
  Allocator* allocator = context->allocator();
  auto* workers_pool = context->workers_pool();

  std::cout << "MultiTHreadGemm 3\n";
  ...
  PackedSideBlock<typename KernelFormat::Rhs> packed_rhs(Side::Rhs, allocator,
                                                         block_params);
  std::cout << "MultiTHreadGemm 4\n";
  allocator->Commit();
  std::cout << "MultiTHreadGemm 5\n";

  // We loop over large blocks of the RHS.
  for (int c = 0; c < cols; c += block_params.l2_cols) {
  std::cout << "MultiTHreadGemm 6\n";
    int cs = std::min(block_params.l2_cols, cols - c);

...
void SingleThreadGemm(SingleThreadGemmContext* context,
  ...
  std::cout << "SingleThreadGemm 1 " << "\n";
  assert(lhs.cols() == rhs.rows());

  int rows = result->rows();
  int cols = result->cols();
  int depth = lhs.cols();
  std::cout << "SingleThreadGemm 2 " << "\n";
  ...
  PackedSideBlock<typename KernelFormat::Rhs> packed_rhs(Side::Rhs, allocator,
                                                         block_params);
  std::cout << "SingleThreadGemm 7 " << "\n";

  PackedResult packed_result(allocator, block_params);
  std::cout << "SingleThreadGemm 8 " << "\n";

  allocator->Commit();
  std::cout << "SingleThreadGemm 9 " << "\n";

  const bool pack_rhs_once = block_params.l2_cols >= cols;

  std::cout << "SingleThreadGemm 10 " << "\n";
  if (pack_rhs_once) {

...
void DispatchGemmShape(GemmContextType* context,
  ...
  std::cout << "DispatchGemmShape \n";
  std::cout << "rhs.rows : " << rhs.rows() << "\n";
  std::cout << "lhs.cols : " << lhs.cols() << "\n";
  assert(lhs.cols() == rhs.rows());

  std::cout << "result Rows : " << result->rows() << " result Cols : " << result->cols() <<  " lhs depth : " << lhs.cols() <<"\n";
  int rows = result->rows();
  int cols = result->cols();
  int depth = lhs.cols();

  std::cout << "DispatchGemmShape1 \n";
  ...
  std::cout << "DispatchGemmShape2 \n";
  std::cout << "Rows : " << rows << " Cols : " << cols <<  " depth : " << depth <<"\n";
  if (rows < cols) {
    auto transposed_result_map = Transpose(*result);
  std::cout << "DispatchGemmShape3 \n";
